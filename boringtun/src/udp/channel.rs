...
mod fragmentation {
    ...
    const MAX_CONCURRENT_FRAGS: usize = 16;
    ...
    impl Ipv4Fragments {
        ...
        pub fn assemble_ipv4_fragment(
            ...
            let exists = fragments
                .binary_search_by_key(&fragment_offset, |f| f.header.fragment_offset())
                .is_ok();
            if exists {
                log::trace!(
            ...
            fragments.push(ipv4_packet);
            fragments.sort_by_key(|f| f.header.fragment_offset());
