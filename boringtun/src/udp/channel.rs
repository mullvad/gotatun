        ...
        match ip_packet {
            Either::Left(ipv4) => {
                let ipv4 = if ipv4.header.fragment_offset() == 0 && !ipv4.header.more_fragments() {
                    ...
                    log::trace!("IPv4 fragment could not be reassembled, dropping");
                    return Ok(());
                };
...
mod fragmentation {
    ...
    use crate::{packet::Udp, udp::channel::IPV4_HEADER_LEN};
    use std::{
    ...
    impl Ipv4Fragments {
        ...
        pub fn assemble_ipv4_fragment(
            ...
            if more_fragments && ipv4_packet.payload.len() % 8 != 0 {
                log::trace!(
                    "Invalid fragment size: {}, dropping",
