...
fn assemble_ipv4_fragment(
    ...
    log::debug!(
        "Received fragment: offset {}, more_fragments: {}, length: {}",
        fragment_offset,
        header.more_fragments(),
        ipv4_packet.payload.len()
    );
    let id = FragmentId {
        idenfication: ipv4_packet.header.identification.get(),
    ...
    if let Some(fragments) = fragment_map.get_mut(&id) {
            ...
            .eq(fragment_ends.take(fragments.len() - 1))
        {
            ...
            let TryIntoUdpResult::Udp(udp_packet) = bytes
                ...
                log::warn!("Failed to convert bytes to UDP packet after reassembly");
                return None;
            };
            Some(udp_packet)
        } else {
