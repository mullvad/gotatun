use boringtun::packet::{IpNextProtocol, Ipv4, Ipv4Header, Packet, PacketBufPool};
...
fn make_single_fragment(
    ...
    let ipv4 = Ipv4::<[u8]>::mut_from_bytes(&mut packet).unwrap();
    ...
    // This will truncate the remaining packet buffer to the appropriate size.
    let mut packet = packet.try_into_ipvx().unwrap().unwrap_left();
...
fn bench_assemble_ipv4_fragment(c: &mut Criterion) {
    ...
    let id = 42;
    let mtu = 1500;
    let mut pool = PacketBufPool::new(MAX_PACKET_BUFS);
    let mut group = c.benchmark_group("assemble_ipv4_fragment");
    for &payload_len in &[2000, 4000, 10000] {
        let payload = vec![0u8; payload_len];
        let frags = fragment_ipv4_packet(id, &payload, mtu, &mut pool);
...
fn bench_assemble_ipv4_fragment_reverse_order(c: &mut Criterion) {
    ...
    for &payload_len in &[2000, 4000, 10000] {
        let payload = vec![0u8; payload_len];
        let mut frags = fragment_ipv4_packet(id, &payload, mtu, &mut pool);
        frags.reverse();
        group.throughput(criterion::Throughput::Bytes(payload_len as u64));
        group.bench_with_input(
            BenchmarkId::from_parameter(payload_len),
