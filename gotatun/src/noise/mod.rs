...
impl<R: RngCore + Send> Tunn<R> {
    ...
    /// sending counter has reached `REJECT_AFTER_MESSAGES`. In the latter case the caller queues
    /// the packet and starts a fresh handshake, so the nonce can never wrap.
