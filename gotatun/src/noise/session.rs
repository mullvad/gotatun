...
/// session key, per the WireGuard whitepaper §6.2 (`Reject-After-Messages = 2^64 - 2^13 - 1`).
...
impl Session {
    ...
    pub(super) fn receive_packet_data(
        ...
        // Refuse counters at or beyond REJECT_AFTER_MESSAGES. A valid peer never sends these.
        // Rejecting before decryption keeps us in lockstep with the spec (§6.2).
