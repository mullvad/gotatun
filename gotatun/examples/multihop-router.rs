...
use gotatun::{
    ...
    tun::{
        IpRecv, merge::MergingIpRecv, router::TunRxRouter, router::TunTxRouter,
...
fn parse_key<T: From<[u8; 32]>>(b64: &str) -> eyre::Result<T> {
    let bytes = base64::engine::general_purpose::STANDARD
        .decode(b64)
        .context("invalid base64 for key")?;
...
async fn main() -> eyre::Result<()> {
    ...
    let packet_pool = PacketBufPool::new(100);
    ...
    let mut router = TunRxRouter::new(tun.clone());
    let default_output = router.add_default_route(4000);
    let alt_output = router.add_route(inner_allowed_ip, 4000);
    let router_task = tokio::spawn(router.run(packet_pool.clone()));
